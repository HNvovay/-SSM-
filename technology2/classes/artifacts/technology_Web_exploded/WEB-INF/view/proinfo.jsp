...
<c:if test="${!empty myproinfo}" var="itinfo">
    ...
    <form method="post" role="form" class="form-comment" action="${pageContext.request.contextPath }/comment/addComment">
        <div class="content">
            <div class="center">
                <p>评论发布</p>
                <input type="hidden" name="pid" value="${myproinfo.pid }">
                <textarea class="comment" autocomplete="off" name="descb" maxlength="300"></textarea>
                <input type="button" class="send" value="发 布" disabled>
            </div>
            <div class="message">
                <c:forEach items="${commentlist}" var="items">
                    <div class="info">
                        <p class="infoText" style="word-wrap: break-word;word-break: break-all;overflow: hidden;">
                                ${items.uname} : ${items.descb }
                        </p>
                        <p class="infoTime"><fmt:formatDate value="${items.commentdate}"
                                                            pattern="yyyy年MM月dd日"/></p>
                        <c:if test="${uid==items.uid}">
                            <p class="deleteInfo" onclick="delComment(${items.commentid})">删除</p>
                        </c:if>
                    </div>
                </c:forEach>
            </div>
